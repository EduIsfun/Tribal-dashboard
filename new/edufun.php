<?php 
	...
	include('classes/Teacher.class.php');
	global $conn;
	$teacher = new Teacher();
	//print_r($_SESSION['currentusernew']);
	$currentUserid=$_SESSION['currentusernew']['userID'];
	$result = $teacher->getStudentresult($currentUserid);
	$studentobj = mysqli_fetch_object($result);
	$classinfo=$teacher->getClass($currentUserid);
?>
...
<body>
	...
	<div class="container-fluid">
		...
		<?php
		$questiontime = "SELECT SUM(timePerQuestion)/COUNT(timePerQuestion) AS `timePerQuestion` FROM knowledgekombat_chapter_time  WHERE  timePerQuestion > 0 AND userID ='".$currentUserid."'";		
		...
		<div class="col-md-6">
			...
			<?php 
			$tqasql = "select SUM(total_questions_answered) AS Totals from knowledgekombat_chapter_time  where userID='".$currentUserid."'";		
			...
			<li class="col-md-4 col-sm-12 col-xs-12">
			<?php 					
			$overallquery  = "SELECT ROUND(SUM(knowledgeCurrency)) as knowledgeCurrency,count(knowledgeCurrency) as outof_knowledgeCurrency FROM `knowledgekombat_treasure_attempt` WHERE `userID` ='".$currentUserid."'";	
	...
	<div class="section2">
		...
		<ul>
			...
			<li class="col-md-2 col-sm-12 col-xs-12">
				...
				<?php 					
				$overallquery  = "SELECT ROUND(SUM(progress)/COUNT(progress)) as overall_progress FROM `knowledgekombat_chapter_time` WHERE `userID` ='".$currentUserid."'";					
				$overallresult = mysqli_query($conn,$overallquery);                 	
				$overallrow = mysqli_fetch_assoc($overallresult);			
				?>
			...
			<?php
				$totaltime = "SELECT TIME_FORMAT(SEC_TO_TIME(ROUND(SUM(`time`))),'%HH : %iM : %SS') AS `time` FROM knowledgekombat_chapter_time  WHERE  userID='".$currentUserid."'";		
		...
		<!-- <li class="col-md-2 col-sm-12 col-xs-12">
				...
				<?php 
					...
					$query  = "SELECT `userID`,`skillID`,`accuracy`,`timestamp`,`created_timestamp`,`updated_timestamp`
						FROM `knowledgekombat_skill_attempt` WHERE  `userID` ='".$currentUserid."'  and `accuracy`>0 AND `timestamp`  IN (
							SELECT MAX(TIMESTAMP)
							FROM knowledgekombat_skill_attempt
							WHERE `userID` ='".$currentUserid."' GROUP BY skillID
						)";
			...
			<li class="col-md-2 col-sm-12 col-xs-12"> 
				<h4 class="learning_head2">
					<?php 	
						$classrankquery = "SELECT ROUND(SUM(kct.learningCurrency)/COUNT(kct.learningCurrency)) AS rank,kct.userID AS userid,kct.chapterID 
						...
						AND usd.school = '".$classinfo->school."'
						GROUP BY kct.userID HAVING rank !=0 ORDER BY rank DESC";
						...
						$classrank  = array_search($currentUserid, $classrankar); 
			...
			<li class="col-md-2 col-sm-12 col-xs-12"> 
				<h4 class="learning_head2">
					<?php 	
						...
						// 	if ($overallrow['userid'] == $currentUserid ){
						// 		$overallrank = $i;	
						// 	}						
						// 	$i++;	
						// }
						...
						FROM knowledgekombat_chapter_time kct 
						INNER JOIN `user_school_details` usd ON kct.`userID`= usd.`userID` 
						WHERE kct.time>0 AND usd.grade='".$classinfo->grade."'
						...
						$globalrank  = array_search($currentUserid, $globalrankar); 
			...
			<li class="col-md-2 col-sm-12 col-xs-12">
			 ...
			 <div class="time">
				...
				<?php					
					$query  = "SELECT  ROUND(SUM(learningCurrency)) as  learningCurrency FROM `knowledgekombat_chapter_time` WHERE `userID` ='".$currentUserid."'";		
					$result = mysqli_query($conn,$query);					
					...
					{	
						...
						$oflc_query  = "SELECT count(*)*100 as outof_learningCurrency FROM  `knowledgekombat_skill` where chapterID IN(SELECT chapterID FROM `knowledgekombat_chapter_time` WHERE userId='".$currentUserid."')";					
...
<div class="table-responsive">  
	...
	WHERE `userID` ='".$currentUserid."' GROUP BY s.`subjectID`  ORDER BY s.`sequence` ASC";
	...
	<tr style="background:#acc61b;">
		...
		INNER JOIN `chapter` c ON kct.`chapterID` = c.`chapterID` 
		INNER JOIN `subject` s ON c.`subjectID` = s.`subjectID`  
		WHERE `userID` ='".$currentUserid."' AND s.subjectID='".$rowsubject->subjectID."' ORDER BY s.`sequence` ASC";
		...
		<?php 	
			$progresssql = "select ROUND(SUM(progress)/COUNT(progress)) as  progress  from knowledgekombat_chapter_time  where chapterID ='".$row1['chapterID']."' and userID='".$currentUserid."'";		
		...
		FROM knowledgekombat_chapter_time  where chapterID ='".$row1['chapterID']."' and userID='".$currentUserid."'";		
        ...
        <td class="text-center" style="width: 105px;">
        	<span class="score">
				<?php 	
				$scoresql  = "SELECT knowledgeCurrency FROM `knowledgekombat_treasure_attempt` WHERE chapterID ='".$row1['chapterID']."' AND `userID` ='".$currentUserid."'";	
        ...
        <td class="text-center" style="width: 105px;">
			<span class="score">
				<?php 	
				//$scoresql = "SELECT ROUND(SUM(learningCurrency)/COUNT(learningCurrency)) as  learningCurrency  from knowledgekombat_chapter_time  where chapterID ='".$row1['chapterID']."' and userID='".$currentUserid."'";		
				...
				// WHERE KSA.userID = '".$currentUserid."' AND KSA.skillID IN (SELECT skillID FROM knowledgekombat_skill WHERE chapterID = '".$row1['chapterID']."') 
				...
				WHERE ksa.userID='".$currentUserid."' AND ksa.skillID IN (SELECT skillID FROM knowledgekombat_skill WHERE chapterID='".$row1['chapterID']."') AND ksa.learningCurrency >0 GROUP BY ksa.skillID ORDER BY ks.skill ASC";
        ...
        <!-- <td style="width: 105px;">
        	<?php 
				...
				$chapterrank  = array_search($currentUserid, $chapterrankr); 
	   ...
	   //          WHERE chapterID ='".$row1['chapterID']."') ) AS rank FROM knowledgekombat_chapter_time where chapterID ='".$row1['chapterID']."' and userID='".$currentUserid."'";
				// echo $ranksql;
