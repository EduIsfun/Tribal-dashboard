...
if (isset($_REQUEST['action']) && ($_REQUEST['action']=="getList")){
	...
	$playdate = isset($_POST['playdate'])?$_POST['playdate']:'';
	...
	$html.= '<table id="example1" class="table table-bordered table-striped" data-page-length="25">
				<thead>
					<tr>
						...
						<th>Date &nbsp <i class="fa fa-caret-down" aria-hidden="true"></i></th>
					...
					$user = $teacher->getuserResult($schoolid,$classid,$subchapterid,$state,$city_id,$board_id,$chapter_id,$subject_id,$playdate);
					if($user){
						$count = 1;
					}
					$counget=1;
					...
					if(mysqli_num_rows($user)>0){
						...
						$gradeid = Romannumeraltonumber($classid);
						$globalresult = $teacher->getglobalrankResult($gradeid,$schoolid,$city_id,$board_id);	
						$globalrankar =  array();
						$totalgrank=0;
						...
						while( $userlist = mysqli_fetch_object($user) ){
							...
							$html.='<tr>
									...
									<td data-sort='.strtotime($userlist->playdate).'><span class="span_inline">'.date('d-m-Y H:i:s',strtotime($userlist->playdate)).'</span></td>
									<td>
									...
									if ($classrankVAL>0) {
										$html.='<td class="orange"><span>'.$classrankVAL.'/'.$totalcrank.'</span></td>';
									} else{
										$html.='<td class="orange"><span>N/A</span></td>';
									}
									if ($globalranks>0) {
										$html.='<td class="orange"><span>'.$globalranks.'/'.$totalgrank.'</span></td>';
									} else{
										$html.='<td class="orange"><span>N/A</span></td>';
									}	
	...
	$html .= '<table id="example1" class="table table-bordered table-striped" data-page-length="25">
			...
			$ids ='';
					...
					if(mysqli_num_rows($user)>0){
						...
						$coloravg='';
						$gradeid = Romannumeraltonumber($classid);
						$overallresult = $teacher->getrankuserResult($classid,$schoolid,$city_id,$board_id,$subject_id);
						$classrankar =  array();
						$i=1;
						while($classrank = mysqli_fetch_object($overallresult)){
						...
						$globalresult = $teacher->getglobalrankResult($schoolid,$city_id,$board_id,$subject_id);	
						$globalrankar =  array();
						$j=1;
						while($globalrank = mysqli_fetch_object($globalresult)){
