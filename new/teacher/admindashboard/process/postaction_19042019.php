 ...
 include('../db_config.php');
	...
	function Romannumeraltonumber($input_roman){
		...
		if (isset($_REQUEST['action']) && ($_REQUEST['action']=="getBoard")){
			...
			$sql ="SELECT usd.school as school  FROM  `knowledgekombat_chapter_time` kct 
			INNER JOIN `user` u ON u.userID = kct .userID 
			INNER JOIN user_school_details usd ON kct.`userID`=usd.userID where 1=1";

			if ($board_id == 'All' ){
				$sql.=" And usd.city_id = '$city' GROUP BY school ";				
			}else if(!empty($city) && $board_id==''){
				$sql.=" And usd.city_id ='$city' OR usd.board_id='' GROUP BY school";		
			}else{
	...
	$html.= '<table id="example1" class="table table-bordered table-striped" data-page-length="25">
				<thead>
					...
					if(mysqli_num_rows($user)>0){
						$ids = '';
						$i=1;
						$overallresult = $teacher->getrankuserResult($classid,$schoolid,$city_id);
						$classrankar =  array();	
						...
						$globalresult = $teacher->getglobalrankResult($classid,$schoolid,$city_id);	
						$globalrankar =  array();
						$j=1;
	...
	$html .= '<table id="example1" class="table table-bordered table-striped" data-page-length="25">
				...
				<thead>
					...
					if(mysqli_num_rows($user)>0){
						...
						$globalresult = $teacher->getglobalrankResult($schoolid,$city_id);	
						$globalrankar =  array();
						$j=1;
						while($globalrank = mysqli_fetch_object($globalresult)){
							$globalrankar[$j] =  $globalrank->userid;
