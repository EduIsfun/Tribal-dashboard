<?php  
		...
		function outputCSV($data) {
			...
			$headers= 'Name, Rank, Global Rank, Time Spent ,Grade, Mobile,Email, Learning Currency(Avg..)';
			$headers.= "\n";
			...
			fclose($outstream);
					...
					$user = $teacher->getuserResult($state_id,$city_id,$board_id,$schoolid,$classid,$subject_id,$chapter_id,$subchapterid);
						...
						while( $userlist = mysqli_fetch_object($user) ){
							...
							if (mysqli_num_rows($overallgrade)>0) {
								if (($allgrade->currency=="") || ($allgrade->currency=="0")) {
									...
									$learningCurrency = $userlist->learningCurrency;
									...
									//$learnincurrancys = $classrank->rank;
									...
									$field_name = $studentName .','. $rank .','. $globalranks .','. $totaltime .','. $showlearningavg .','.$studentMobile.','.$studentEmail.','.$learningCurrency.','; 
