	...
	<div class="section1">
		<div class="row">
			<div class="col-md-6">
				...
				<form class="form-horizontal" action="" method="post" id="úserdetails">
			...
			<div class="">
				<ul>
					...
					<li class="col-md-2 col-sm-12 col-xs-12"> 
						...
						<div class="time">
					<?php 	
					$classrankquery="SELECT userid, totalcount, rank FROM ( SELECT userid, totalcount, 
					@r := IF(@c = totalcount, @r, @r + 1) rank, @c := totalcount  FROM ( SELECT  ksa.userID, 
					ROUND(SUM(ksa.learningCurrency)/COUNT(ksa.learningCurrency)) AS totalcount FROM
					knowledgekombat_skill_attempt  ksa INNER JOIN `user_school_details` usd ON 
					ksa.`userID`=usd.`userID` INNER JOIN knowledgekombat_skill ks ON ksa.skillID=ks.skillID INNER JOIN
					chapter c ON c.chapterID=ks.chapterID ";    
					if (!empty($classinfo->school_id)){
					...
					<div class="circle">
						<div class="time">
					<?php 	
					$globalrankquery="SELECT userid, totalcount, rank FROM ( SELECT userid,totalcount,
					@r:=IF(@c=totalcount,@r,@r+1) rank, @c := totalcount FROM (SELECT  ksa.userID, 
					ROUND(SUM(ksa.learningCurrency)/COUNT(ksa.learningCurrency)) AS totalcount FROM
					...
					$globalrankquery.=" WHERE usd.city_id ='".$cityinfo->id."' ";//AND c.grade ='".$selectgrade."' ";
					if (!empty($classinfo->school_id)){
