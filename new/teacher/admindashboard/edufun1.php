<?php 
	...
	$classinfolist=$teacher->getClassList($userID);
		 ...
		 <form class="form-horizontal" action="" method="post">
            ...
            <div class="form-group">
			 ...
			 <div class="form-group">
			  ...
			  <label class="col-md-1 control-label">:</label>
              <div class="col-md-9">
			  <?php while($classinfolistobj = mysqli_fetch_object($classinfolist)){ ?>
				  
				 <select  id="name" name="name" >
				 <option value="<?php echo $classinfolistobj->grade;?>"><?php echo $classinfolistobj->grade;?></option>
				 </select>
	...
	<tr style="background:#acc61b;">
	   ...
	   //          WHERE chapterID ='".$row1['chapterID']."') ) AS rank FROM knowledgekombat_chapter_time where chapterID ='".$row1['chapterID']."' and userID='".$currentUser."'";
      	...
      	<tr>
			...
			INNER JOIN knowledgekombat_skill as ks ON ksa.skillID=ks.skillID
			WHERE ksa.userID='".$currentUser."' AND ksa.skillID IN (SELECT skillID FROM knowledgekombat_skill WHERE chapterID='".$row1['chapterID']."') AND ksa.learningCurrency >0 GROUP BY ksa.skillID ORDER BY ks.skillID ASC";
			//echo $sql; 
				...
				<div id="accordion_<?php echo $acc; ?>" class="collapse">
					<table class="table table-bordered" style="margin-bottom: 0px !important; width: 100%;">
						...
						while($islanddetailobj = mysqli_fetch_object($islanddetailresult)){
							...
							$sql="SELECT `timeperquestion` as questiontime FROM  `knowledgekombat_skill_attempt` WHERE skillID=".$islanddetailobj->skillID ." ORDER BY `timestamp` DESC LIMIT 1";	
