...
public function getuserResult($schoolid,$classid,$subchapterid,$state,$city,$board_id,$chapter_id,$subject_id,$playdate){
	...
	global $conn;
		...
		$sql ="SELECT  kct.`userID`,u.userID, u.name AS student_name,usd.school, usd.grade,u.updated_timestamp as playdate FROM `knowledgekombat_chapter_time` kct
	...
	public function getrankuserResult($classid,$schoolname,$city_id,$board_id)
	{
		...
		*/	
			$query="SELECT userid, totalcount, rank FROM (
				...
				SELECT  kct.userID, ROUND(SUM(kct.learningCurrency)/COUNT(kct.learningCurrency))
				AS totalcount,kct.chapterID FROM knowledgekombat_chapter_time kct 
				INNER JOIN `user_school_details` usd ON kct.`userID`= usd.`userID` ";
				if(!empty($board_id)&&$board_id=='4'){
					$query.=" INNER Join school_master sm on sm.school_code= usd.school_id";
				}				
