...
public function getuserResult($schoolid,$classid,$subchapterid,$state,$city,$board_id,$chapter_id,$subject_id,$playdate){
		
	global $conn;

		$sql ="SELECT  kct.`userID`,u.userID, u.name AS student_name,usd.school, usd.grade,ROUND(SUM(kct.learningCurrency)/COUNT(kct.learningCurrency)) AS rank,kta.updated_timestamp as playdate FROM `knowledgekombat_chapter_time` kct
		...
		$sql.=" INNER Join knowledgekombat_treasure_attempt kta on kct.userID= kta.userID";
		...
		if ($playdate!="") {
			if ($playdate==0){
				$today = date('Y-m-d');
				$sql .= " AND Date(kta.updated_timestamp)='$today'";
			}	
			if ($playdate==1){
				$olddate = date('Y-m-d',strtotime("-1 days"));
				$sql .= " AND Date(kta.updated_timestamp)='$olddate'";
			}	
			if ($playdate==7){
				$today = date('Y-m-d');
	...
	{
		global $conn;
		$query = "SELECT kct.userID AS userid, ROUND(SUM(kct.learningCurrency)/COUNT(kct.learningCurrency)) AS rank,kct.chapterID FROM knowledgekombat_chapter_time kct 
		INNER JOIN `user_school_details` usd ON kct.`userID`= usd.`userID` ";
		if(!empty($board_id)&&$board_id=='4'){
			$query.=" INNER Join school_master sm on sm.school_code= usd.school_id";
		}
