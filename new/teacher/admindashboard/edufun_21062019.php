...
<body>
	...
	<div class="container-fluid">
		...
		<?php
		$questiontime = "SELECT SUM(timePerQuestion)/COUNT(timePerQuestion) AS `timePerQuestion` FROM knowledgekombat_skill_attempt  WHERE  timePerQuestion > 0 AND userID ='".$currentUser."'";		
		...
		<!-- <li class="col-md-2 col-sm-12 col-xs-12">
			...
			<li class="col-md-2 col-sm-12 col-xs-12"> 
				<h4 class="learning_head2">
					<?php 	
						...
						//echo $classrankquery;
						*/
						$classrankquery="SELECT userid, totalcount, rank FROM (
										 SELECT userid, totalcount, 
										@r := IF(@c = totalcount, @r, @r + 1) rank, @c := totalcount  
										FROM (
										  SELECT  kct.userID, 
										  ROUND(SUM(kct.learningCurrency)/COUNT(kct.learningCurrency))
										  AS totalcount,kct.chapterID FROM knowledgekombat_chapter_time 
										  kct INNER JOIN `user_school_details` usd ON kct.`userID`=
										  usd.`userID` ";  
						if (!empty($classinfo->school_id)){
							$classrankquery.=" INNER Join school_master sm on sm.school_code= 
							usd.school_id";
						}

						$classrankquery.=" WHERE usd.grade='".$classinfo->grade."' AND usd.city_id =
						'".$cityinfo->id."'"; 
						if (!empty($classinfo->school_id)){
							$classrankquery.="  AND sm.school = '".$schoolname."'";
						} else {
						...
						$classrankquery.="  GROUP BY userid ORDER BY totalcount DESC  ) t CROSS JOIN 
			...
			<li class="col-md-2 col-sm-12 col-xs-12"> 
				<h4 class="learning_head2">
					<?php 	
						...
						$globalrankquery="SELECT userid, totalcount, rank FROM (
										 SELECT userid, totalcount, 
										  @r := IF(@c = totalcount, @r, @r + 1) rank, @c := totalcount  
										  FROM (
										  SELECT  kct.userID, 
						...
						$globalrankquery.=" WHERE usd.grade='".$classinfo->grade."' AND usd.city_id =
						'".$cityinfo->id."' GROUP BY userid ORDER BY totalcount DESC  ) t CROSS JOIN 
										  ( SELECT @r := 0, @c := NULL  ) i ) q";
			...
			<li class="col-md-2 col-sm-12 col-xs-12">
			 ...
			 <div class="time">
				...
				<?php	
					//$query  = "SELECT  * FROM `knowledgekombat_skill_attempt` WHERE `userID` ='".$currentUser."' ORDER BY `timestamp` DESC LIMIT 1";	
					$query ="SELECT * FROM knowledgekombat_skill_attempt WHERE `userID` ='".$currentUser."'  AND `skillAttemptID` IN ( SELECT MAX(`skillAttemptID`) FROM `knowledgekombat_skill_attempt` WHERE `userID` ='".$currentUser."' GROUP BY `skillID`)";
	...
	<tr style="background:#acc61b;">
		...
		<td style="width: 100px">
			<?php 
				//$islandsql = "SELECT count(*) as island FROM knowledgekombat_skill WHERE chapterID='".$row1['chapterID']."'";
				 $islandsql = "SELECT DISTINCT(ksa.skillID) FROM knowledgekombat_skill_attempt AS ksa INNER JOIN knowledgekombat_skill AS ks ON ksa.skillID=ks.skillID WHERE ksa.userID='".$currentUser."' AND ks.chapterID='".$row1['chapterID']."' AND ksa.learningCurrency >0 ";
        ...
        <td class="text-center" style="width: 105px;">
			<span class="score">
				...
				$learningCurrencysql="SELECT ksa.learningCurrency AS lc FROM
				knowledgekombat_skill_attempt AS ksa INNER JOIN knowledgekombat_skill AS ks ON
				ksa.skillID=ks.skillID WHERE ksa.userID='".$currentUser."' AND ksa.skillID IN (SELECT
				skillID FROM knowledgekombat_skill WHERE chapterID='".$row1['chapterID']."')  AND
				ksa.skillAttemptID IN ( SELECT MAX(`skillAttemptID`) FROM knowledgekombat_skill_attempt
				WHERE `userID` ='".$currentUser."' GROUP BY `skillID`)";
	   ...
	   //          WHERE chapterID ='".$row1['chapterID']."') ) AS rank FROM knowledgekombat_chapter_time where chapterID ='".$row1['chapterID']."' and userID='".$currentUser."'";
      	...
      	<tr>
			...
			INNER JOIN knowledgekombat_skill as ks ON ksa.skillID=ks.skillID
			WHERE ksa.userID='".$currentUser."' AND ksa.skillID IN (SELECT skillID FROM knowledgekombat_skill WHERE chapterID='".$row1['chapterID']."')  GROUP BY ksa.skillID ORDER BY ks.skillID ASC";
			//AND ksa.learningCurrency >0
			//echo $sql; 
				...
				<div id="accordion_<?php echo $acc; ?>" class="collapse">
					<table class="table table-bordered" style="margin-bottom: 0px !important; width: 100%;">
						...
						while($islanddetailobj = mysqli_fetch_object($islanddetailresult)){
							...
							$learningCurrencytopic="SELECT ksa.learningCurrency AS lc FROM
							knowledgekombat_skill_attempt AS ksa WHERE ksa.userID='".$currentUser."' AND
							ksa.skillID = '".$islanddetailobj->skillID."' AND ksa.skillAttemptID IN (
							SELECT MAX(`skillAttemptID`) FROM knowledgekombat_skill_attempt WHERE
							`userID` ='".$currentUser."' GROUP	BY `skillID`)";
							$lCresult = mysqli_query($conn,$learningCurrencytopic);
							$lCresultrow = mysqli_fetch_object($lCresult);
							
							//$islandwiselearningCurrency=$islanddetailobj->learningCurrency;
							$islandwiselearningCurrency=$lCresultrow->lc;
							...
							$sql="SELECT `timeperquestion` as questiontime FROM  `knowledgekombat_skill_attempt` WHERE skillID=".$islanddetailobj->skillID ." and userID='".$currentUser."' ORDER BY `timestamp` DESC LIMIT 1";	
							$questiontimeresult = mysqli_query($conn,$sql);
			  	          	$questiontimeresultobj = mysqli_fetch_object($questiontimeresult);
			  	          	if (($questiontimeresultobj->questiontime!='')&&($questiontimeresultobj->questiontime!=0)) {
			  	          		echo "<td>".$questiontimeresultobj->questiontime." s </td>";
			  	          	} else {
								/*$sqlq="SELECT `timePerQuestion` as questiontime FROM  `knowledgekombat_chapter_time` WHERE chapterID='".$row1['chapterID'] ."' and userID='".$currentUser."'   LIMIT 1";	
								$questimePerQuestion = mysqli_query($conn,$sqlq);
								$timePerQuestionsultobj = mysqli_fetch_object($questimePerQuestion);
								if (($timePerQuestionsultobj->questiontime!='')&&($timePerQuestionsultobj->questiontime!=0)) {
			  	          		echo "<td>".$timePerQuestionsultobj->questiontime." s </td>";
							} else {*/
								echo "<td>N/A </td>";
							}
