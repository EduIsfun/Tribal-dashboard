		...
		<div class="row">
			<div class="">
				<ul>
					...
					<h2 class="learning_head">CLASS <?php echo $classinfo->grade;?>
					</h2>
					...
					SELECT userid, totalcount, @r := IF(@c = totalcount, @r, @r +
					1) rank, @c := totalcount  FROM ( SELECT  ksa.userID, 
					ROUND(SUM(ksa.learningCurrency)/COUNT(ksa.learningCurrency))
					AS totalcount FROM knowledgekombat_skill_attempt 
					ksa INNER JOIN `user_school_details` usd ON
					ksa.`userID`=usd.`userID` ";  
					if (!empty($classinfo->school_id)){
						$classrankquery.=" INNER Join school_master sm on
						sm.school_code= usd.school_id";
					}
					$classrankquery.=" WHERE usd.grade='".$classinfo->grade."' AND
					usd.city_id ='".$cityinfo->id."'"; 
					if (!empty($classinfo->school_id)){
					...
					$classrankquery.="  GROUP BY userid ORDER BY totalcount DESC) t
					CROSS JOIN ( SELECT @r := 0, @c := NULL  ) i ) q";
					...
					SELECT userid, totalcount,@r:=IF(@c=totalcount,@r,@r+1) rank,
					@c := totalcount FROM (SELECT  ksa.userID, 
					ROUND(SUM(ksa.learningCurrency)/COUNT(ksa.learningCurrency))
					AS totalcount FROM knowledgekombat_skill_attempt 
					ksa INNER JOIN `user_school_details` usd ON
					ksa.`userID`=usd.`userID` ";  
					...
					$globalrankquery.=" WHERE usd.city_id ='".$cityinfo->id."'";
					if (!empty($classinfo->school_id)){
						$globalrankquery.="  AND sm.school = '".$schoolname."'";
					} else {
						$globalrankquery.="  AND usd.school = '".$schoolname."'";
					}
					
					$globalrankquery.=" GROUP BY userid ORDER BY
					totalcount DESC) t CROSS JOIN (SELECT @r:=0,@c:=NULL)i) q";
					...
					<div class="circle">
						<div class="time">
						<?php					
						$query ="SELECT * FROM knowledgekombat_skill_attempt WHERE
						userID ='".$currentUser."' AND skillAttemptID IN (SELECT 
						MAX(skillAttemptID) FROM knowledgekombat_skill_attempt
...
<div class="container-fluid">
	<div class="table-responsive">  
		<table class="table table-bordered main_table">
			...
			<tbody>
			<?php	 
			$sqlsubject = "SELECT kct.*,c.*,s.* FROM `knowledgekombat_chapter_time`
			kct INNER JOIN `chapter` c ON kct.`chapterID` = c.`chapterID` INNER
			JOIN `subject` s ON c.`subjectID` = s.`subjectID`  WHERE 
			`userID` ='".$currentUser."' GROUP BY s.`subjectID` ORDER BY
			s.`sequence` ASC";
				...
				?>
					...
					<div id="accordion_<?php echo $acc; ?>" class="collapse">
						...
						<?php
							...
							$lCurrencytopic="SELECT ksa.learningCurrency AS lc FROM
							knowledgekombat_skill_attempt AS ksa WHERE 
							ksa.userID='".$currentUser."' AND
							ksa.skillID = '".$isdetailobj->skillID."' 
							AND ksa.skillAttemptID IN (	SELECT
							MAX(`skillAttemptID`) FROM 
