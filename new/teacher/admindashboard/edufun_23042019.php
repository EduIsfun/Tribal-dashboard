			...
			<li class="col-md-2 col-sm-12 col-xs-12">
			 ...
			 <div class="time">
				...
				<?php	
					...
					$query  = "SELECT  ROUND(SUM(learningCurrency)) as  learningCurrency FROM `knowledgekombat_skill_attempt` WHERE `userID` ='".$currentUser."'";		
					$result = mysqli_query($conn,$query);					
...
<div class="table-responsive">  
	...
	<tr style="background:#acc61b;">
		...
		INNER JOIN `chapter` c ON kct.`chapterID` = c.`chapterID` 
		INNER JOIN `subject` s ON c.`subjectID` = s.`subjectID`  
		WHERE `userID` ='".$currentUser."' AND s.subjectID='".$rowsubject->subjectID."' ORDER BY s.`sequence` ASC";
		...
		<td style="width: 100px">
			<?php 
				...
				$islandsql = "SELECT COUNT(*) AS island FROM knowledgekombat_skill_attempt AS ksa INNER JOIN knowledgekombat_skill AS ks ON ksa.skillID=ks.skillID WHERE ksa.userID='".$currentUser."' AND ks.chapterID='".$row1['chapterID']."'";
