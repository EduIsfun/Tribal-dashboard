...
class Dashboard
{
	...
	
	{
		...
		global $conn;
		$query = "SELECT nk.skillID,nk.chapterID,nk.skill,u.userID,MAX(nka.timestamp), ROUND(((nka.learningCurrency)*20/100)+((kta.knowledgeCurrency)*80/1000))  AS learning
		FROM `knowledgekombat_skill_attempt` nka INNER JOIN `knowledgekombat_skill` nk ON nk.skillID=nka.skillID INNER JOIN `user` u ON u.userID = nka.userID 
		LEFT JOIN knowledgekombat_treasure_attempt kta ON (kta.userID= nka.userID)
