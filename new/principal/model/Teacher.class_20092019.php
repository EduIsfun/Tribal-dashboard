...
class  Teacher
{
	...
	public function getrankuserResult($classid,$schoolname,$city_id,$chapter_id,$subtopic,$subject_id)
	{
		...
		$query="SELECT userid, totalcount, rank FROM (
				...
				SELECT  ksa.userID, ROUND((SUM(ksa.learningCurrency)*20/100)+(SUM(kta.`knowledgeCurrency`)*80/100)) AS totalcount FROM knowledgekombat_skill_attempt ksa 
	...
	public function getoverallGrade($userid){
		...
		GROUP BY userID ";*/
			$query = "SELECT learningCurrency AS currency, userID AS userid FROM knowledgekombat_skill_attempt WHERE userID='".$userid."'  ORDER BY MAX(TIMESTAMP) DESC LIMIT 0,1";
		//echo $query."<br><br>";
		$result = mysqli_query($conn,$query);
