...
class Dashboard
{
	...
	
	{
		...
		nka.`learningCurrency` AS learning FROM `knowledgekombat_skill_attempt` nka INNER JOIN `knowledgekombat_skill` nk ON nk.skillID=nka.skillID INNER JOIN `user` u ON u.userID = nka.userID WHERE nk.skillID='".$skillid."' and u.userID='".$userid."'
		AND skillAttemptID IN
		( SELECT MAX(`skillAttemptID`) FROM knowledgekombat_skill_attempt ksa
		WHERE ksa.userID ='".$userid."' and  ksa.skillID = '".$skillid."' GROUP BY `skillID`)
		";
	...
	function getlearningResultAverage($chapter_id,$schoolname,$classid,$subject_id,$state_id,$city_id,$skillid){
		...
		// $query .= "and  nka.skillAttemptID IN
		// ( SELECT MAX(`skillAttemptID`) FROM knowledgekombat_skill_attempt ksa
		// WHERE ksa.userID ='".$userid."' and  ksa.skillID = '".$skillid."' GROUP BY `skillID`) ";
		$query .= " ORDER BY MAX(nka.created_timestamp) DESC LIMIT 0,1;";
