...
class  Teacher
{
	...
	public function getrankuserResult($classid,$schoolname,$city_id,$chapter_id,$subtopic,$subject_id)
	{
		...
		$query="SELECT userid, totalcount, rank FROM (
				...
				SELECT  ksa.userID, ((IFNULL(((AVG(ksa.learningCurrency)*20)/100),0))+ (IFNULL(((AVG(kta.knowledgeCurrency)*80)/1000),0))) AS totalcount FROM knowledgekombat_skill_attempt ksa 
	...
	public function getglobalrankResult($classid,$schoolname,$city_id,$chapter_id,$subtopic,$subject_id)
	{
		...
		$query.=" SELECT ksa.`userID`,((IFNULL(((AVGAearningCurrency*20)/100),0))+ (IFNULL(((AVGknowledgeCurrency*80)/1000),0))) 
	...
	public function getaliasrankResult($classid,$schoolname,$city_id,$chapter_id,$subtopic,$subject_id)
	{
		global $conn;
		$query="SELECT userid, totalcount, rank FROM (
				SELECT userid, totalcount, 
				@r := IF(@c = totalcount, @r, @r + 1) rank, @c := totalcount  FROM (
				SELECT  ksa.userID, ((IFNULL(((AVG(ksa.learningCurrency)*20)/1000),0))+ (IFNULL(((AVG(kta.knowledgeCurrency)*80)/1000),0))) AS totalcount FROM knowledgekombat_skill_attempt ksa 
				INNER JOIN `user_school_details` usd ON ksa.`userID`= usd.`userID` 
				left JOIN `knowledgekombat_treasure_attempt` kta ON kta.`userID`= usd.`userID` ";
		if (!empty($subject_id)){	
			$query.=" INNER JOIN knowledgekombat_skill ks ON ksa.skillID=ks.skillID INNER JOIN chapter c ON
