...
class  Teacher
{
	...
	public function getoverallGradeNew($subtopicid,$subject_id, $user_ids = '', $grade = '', $schoolname = '') {
		...
		$subQuery = " SELECT  MAX(skillAttemptID)  FROM knowledgekombat_skill_attempt ksa use index (skillAttemptID) ";
		...
		$query= " select  AVG(ksa.learningCurrency)  AS currency, userID as userid from  knowledgekombat_skill_attempt ksa where skillAttemptID in ( ";
		$query .= $subQuery . $where . $groupby . $orderby ;
		$query .= " )  ";
