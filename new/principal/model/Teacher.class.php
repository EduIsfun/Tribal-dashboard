...
class  Teacher
{
	...
	public function getrankuserResultNew($classid,$schoolname,$city_id,$chapter_id,$subtopic,$subject_id, $userIDs = array())
	{

		global $conn;

		$subQuery = " SELECT  MAX(skillAttemptID) as skillAttemptID FROM knowledgekombat_skill_attempt ksa use index (skillAttemptID) ";
		$where = " WHERE  ksa.school in ('".$schoolname."')  ";
		$groupby = " GROUP BY userID, skillID ";
		$orderby = " ";


		if (!empty($userIDs) && $userIDs != ''){	
			$where.= " AND ksa.userID in ('".$userIDs."')  ";
		}

		if (!empty($subtopic) && $subtopic != ''){	
			$where.= " AND ksa.skillID = '$subtopic' ";
		}

		if (!empty($grade) && $grade != '' && $grade != 'all'){	
			$where.= " AND ksa.grade = '$grade' ";
		}
		...
		$q = $subQuery . $where . $groupby . $orderby;
		...
		{
			$set[] = $row['skillAttemptID'];
		}

		$implodedSet = implode(',', $set);
		$query = " select  AVG(ksa.learningCurrency)  AS totalcoun, userID as userid from  knowledgekombat_skill_attempt ksa where skillAttemptID in ( " .$implodedSet ." )" ;
		...
		$query = $query. " group by userID ORDER BY totalcoun DESC  ";
	...
	public function getoverallGradeNew($subtopicid,$subject_id, $user_ids = '', $grade = '', $schoolname = '') {


		// select AVG(learningCurrency) AS currency, userID as userid from knowledgekombat_skill_attempt where skillAttemptID in ( SELECT MAX(skillAttemptID) FROM knowledgekombat_skill_attempt ksa use index (skillAttemptID) WHERE ksa.school in ('EMRS, Ajmer Saundane','EMRS, Borgaon','EMRS, Chamorshi','EMRS, Chikhaldara','EMRS, Dewada','EMRS, Kambalgaon','EMRS, Khairi-Parsoda','EMRS, Maveshi','EMRS, Nandurbar','EMRS, Nashik','EMRS, Pimpalner','EMRS, Pimprisadroddin','EMRS, Sahastrakund','EMRS, Shendegaon','EMRS, Titve','EMRS, Tumargunda') GROUP BY userID, skillID )  group by userID ORDER BY `created_timestamp` DESC
		...
		if (!empty($user_ids) && $user_ids != ''){	
			$where.= " AND ksa.userID in ('".$user_ids."')  ";
		}
		...
		$implodedSet = implode(',', $set);
		$query = " select  AVG(ksa.learningCurrency)  AS currency, userID as userid from  knowledgekombat_skill_attempt ksa where skillAttemptID in ( " .$implodedSet ." )" ;
		...
		$query = $query. " group by userID ORDER BY ksa.`created_timestamp` DESC  ";
		// echo $query;
		// exit;
		return $result = mysqli_query($conn,$query);
		...
		$subQuery = " SELECT  (IFNULL(((AVG(ksa.learningCurrency)*20)/100),0)) as currency, ksa.userID, ksa.created_timestamp, ksa.skillID  FROM knowledgekombat_skill_attempt ksa use index (userID_3) ";
		$where = " WHERE  ksa.userID in ('".$user_ids."') and ksa.skillAttemptID IN ( select MAX(skillAttemptID) from knowledgekombat_skill_attempt WHERE  userID in ('".$user_ids."') GROUP BY userID, skillID ) ";
		$groupby = " GROUP BY userID ";
		$orderby = " ORDER BY ksa.`created_timestamp` DESC ";
		...
		$query= " select  t.currency + (IFNULL(((AVG(kta.knowledgeCurrency)*80)/1000),0)) AS currency, t.userID as userid from ( ";
		$query .= $subQuery . $where . $groupby . $orderby ;
		$query .= " ) t left JOIN `knowledgekombat_treasure_attempt` kta use INDEX (userID_2) ON kta.`userID`= t.userID ";

		if (!empty($subject_id)){	
			$query.= " INNER JOIN knowledgekombat_skill ks ON t.skillID=ks.skillID INNER JOIN chapter c ON c.chapterID=ks.chapterID ";
		}
		...
		$query = $query. " group by t.userID ORDER BY t.`created_timestamp` DESC  ";
		// echo $query;
		// exit;
		return $result = mysqli_query($conn,$query);
		...
		$query.= "  WHERE ksa.userID in ('".$user_ids."') ";
		...
		$query.= " group by ksa.userID ORDER BY ksa.`timestamp` DESC ";
		// echo $query;
		// exit;
