	...
	$school_id = $obj->school;
		...
		if (isset($_REQUEST['action']) && ($_REQUEST['action']=="getClass")){
			...
			$output.='<ul class="list-group list-scroll">
						...
						<img src="images/key.png" style="opacity: .6;" alt="" /> &nbsp CHAPTER
						</li>';
			$countchapter='1';
		...
		if (isset($_REQUEST['action']) && ($_REQUEST['action']=="getSubject")){
			...
			if ($classid!="all" && !empty($classid)){
				//$classid =  Romannumeraltonumber( $_POST['classid'] ) ? Romannumeraltonumber($_POST['classid'] ): 1;
				$sql = "SELECT * FROM chapter WHERE subjectID = $subject_id  AND grade = $classid  GROUP BY chapter";
			}
			else
			{
				$sql = "SELECT * FROM chapter WHERE subjectID = $subject_id  GROUP BY chapter";
			}
			$result = mysqli_query($conn,$sql);
...
if (isset($_REQUEST['action']) && ($_REQUEST['action']=="getList")){
   ...
   //die();
	...
	$html.= '<thead>
				...
				<div class="box-tools">
							<a href="download-report.php?state_id='.$_POST['state_id'].'&city_id='.$city_id.'&schoolname='.$schoolname.'&classid='.$classid.'&subject_id='.$subject_id.'" class="btn btn-success button-loading pull-right" 
					...
					$user = $teacher->getuserResult($classid,$subject_id,$schoolname,$state_id,$city_id,$chapter_id,$subtopic);
						...
						$i=1;
						$overallresult = $teacher->getrankuserResult($classid,$schoolname,$city_id);
						$classrankar =  array();	
						...
						// $globalresult = $teacher->getglobalrankResult($schoolname,$city_id);	
						// $globalrankar =  array();
						// $j=1;
						...
						$globalresult = $teacher->getglobalrankResult($classid,$schoolname,$city_id);	
						$globalrankar =  array();
						$j=1;
						while($globalrank = mysqli_fetch_object($globalresult)){
							$globalrankar[$j] =  $globalrank->userid;
							...
							while( $userlist = mysqli_fetch_object($user) ){
								...
								if (mysqli_num_rows($overallgrade)>0) {
									if (($allgrade->currency=="") || ($allgrade->currency=="0")) {
										...
										} else if (($learnincurrancy>=0) && ($learnincurrancy<30)){
											$showlearningavg ="E1";
											$coloravg ="redr";
											$img_grade = 'images/red.png';
										} 
								...
								if ($classrankVAL>0) {
										$html.='<td class="dark"><span>'.$classrankVAL.'</span></td>';
								} else{
				...
				<tr>
					...
					if($user){
					    ...
					    $html.='<div class="box-tools">
							<a href="download-report.php?state_id='.$_POST['state_id'].'&city_id='.$city_id.'&schoolname='.$schoolname.'&classid='.$classid.'&subject_id='.$subject_id.'&chapter_id='.$chapter_id.'&subtopic='.$subtopic.'" class="btn btn-success button-loading pull-right" 
							id="btns" onclick="getExport()";>Export Record</a>
						</div>';
					...
					if(mysqli_num_rows($user)>0){
						...
						$rowskill = mysqli_fetch_object($resultskill);
							...
							$resultlearningaverage = $teacher->getlearningResultAverage($chapter_id,$school_id,$classid,$subject_id,$state_id,$city_id,$subtopic);
							...
							if (mysqli_num_rows($resultlearningaverage)>0) {
								if (($rowlearningaveareg->learningavg=="") || ($rowlearningaveareg->learningavg=="0") ) {
									...
									if ($learningaverage>=90){
										...
										$showlearningavg ="D";
										$coloravg ="blue2";
									} else if (($learningaverage>=0) && ($learningaverage<30)){
										$showlearningavg ="E1";
										$coloravg ="red";
									} 
									$overallavgcount++;
							...
							$i=1;
							$rank = 0;
							$overallresult = $teacher->getrankuserResult($classid,$schoolname);
							while($classrank = mysqli_fetch_object($overallresult)){
							...
							$html.='<td class="orange"><span>'.$classid."/".$globalranks.'</span></td>';
