...
$sqlquery="SELECT MAX(ksa.created_timestamp) as maxtime FROM knowledgekombat_skill_attempt ksa INNER 
JOIN user_school_details usd ON ksa.userID=usd.userID WHERE 
LOWER(usd.school) = '".strtolower($schoolarray)."'";
        ...
        <div class="row">
            ...
            <div class="col-md-4">
				...
				<?php if ($_SESSION['uids']!=""){?>
					...
					<?php if ($lastupdatedtime!='') {?>
                    <div> <strong>Last Updated :</strong> <?php echo $lastupdatedtime;?></div>
					<?php } ?>
					<?php if ($mainschool!=''){ ?>
					<button class="btn btn-primary" id="menu1" type="button" ><?php echo $mainschool;?>    
					<?php } ?>
