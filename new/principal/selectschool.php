...
if(isset($_POST['submit']) AND $_POST['submit'] == 'GO'){
	...
	$schoolname=implode("|",$_POST['selectedvalue']);
		...
		<div class="col-md-8 col-md-offset-2">
			...
			<div class="table-responsive sb_border-shadow">		
				<table class="table inner_tabel">
					<tr> 
					<td><input type="checkbox" name="select-all" id="select-all" /> <strong> Select All</strong></td>
				...
				if(!empty($_SESSION['uids'])){
					...
					?>
					<tr> 
						<td><input type="checkbox" name="selectedvalue[]" value="<?php echo $value; ?>" ></td>
				...
				<?php if ($obj->main_school!=''){?>
						<tr> 
						<td><input type="checkbox" name="selectedvalue[]" value="<?php echo $obj->main_school; ?>" ></td>
...
function formsubmit(){
	var schoolname = document.getElementsByName("selectedvalue[]");
	var chesch =0 ;
	for (var i=0; i<schoolname.length; i++) {
		// If you have more than one radio group, also check the name attribute
		// for the one you want as in && chx[i].name == 'choose'
		// Return true from the function on first match of a checked item
		if (schoolname[i].type == 'checkbox' && schoolname[i].checked) {
			chesch=1;
