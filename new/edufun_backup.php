...
<body>
	...
	<div class="container-fluid">
		...
		<?php
		$questiontime = "SELECT SUM(timePerQuestion)/COUNT(timePerQuestion) AS `timePerQuestion` FROM knowledgekombat_chapter_time  WHERE  timePerQuestion > 0 AND userID ='".$_SESSION['currentusernew']['userID']."'";		
		...
		<div class="col-md-6">
			...
			<?php 
			$tqasql = "select SUM(total_questions_answered) AS Totals from knowledgekombat_chapter_time  where userID='".$_SESSION['currentusernew']['userID']."'";		
			...
			<li class="col-md-4 col-sm-12 col-xs-12">
			<?php 					
			$overallquery  = "SELECT ROUND(SUM(knowledgeCurrency)) as knowledgeCurrency FROM `knowledgekombat_treasure_attempt` WHERE `userID` ='".$_SESSION['currentusernew']['userID']."'";					
...
// $romanValues=array('I' => 1,'II' => 2,'III' => 3,'IX' => 4,'V' => 5,'VI' => 6,'VII' => 7,'VIII' => 8,'IX' => 9,'X' => 10,'XI' => 11,'XII' => 12,);	
			...
			<li class="col-md-2 col-sm-12 col-xs-12">
				...
				<?php 					$overallquery  = "SELECT ROUND(SUM(progress)/COUNT(progress)) as overall_progress FROM `knowledgekombat_chapter_time` WHERE `userID` ='".$_SESSION['currentusernew']['userID']."'";					
				$overallresult = mysqli_query($conn,$overallquery);                 	
				$overallrow = mysqli_fetch_assoc($overallresult);			?>
			<div class="circle">
				<div class="time">
			...
			<?php
				$totaltime = "SELECT TIME_FORMAT(SEC_TO_TIME(ROUND(SUM(`time`))),'%HH : %iM : %SS') AS `time` FROM knowledgekombat_chapter_time  WHERE  userID='".$_SESSION['currentusernew']['userID']."'";		
		...
		<li class="col-md-2 col-sm-12 col-xs-12">
				...
				<?php 
					...
					$query  = "SELECT `userID`,`skillID`,`accuracy`,`timestamp`,`created_timestamp`,`updated_timestamp`
						FROM `knowledgekombat_skill_attempt` WHERE  `userID` ='".$_SESSION['currentusernew']['userID']."'  and `accuracy`>0 AND `timestamp`  IN (
							SELECT MAX(TIMESTAMP)
							FROM knowledgekombat_skill_attempt
							WHERE `userID` ='".$_SESSION['currentusernew']['userID']."' 
							GROUP BY skillID
						)";
			...
			<?php 	
				...
				while ($overallrow = mysqli_fetch_assoc($overallresult)){
					if ($overallrow['userid']==$_SESSION['currentusernew']['userID']){
					$overallrank = $i;	
					}	
			...
			<?php					
			 $query  = "SELECT  ROUND(SUM(learningCurrency)) as  learningCurrency FROM `knowledgekombat_chapter_time` WHERE `userID` ='".$_SESSION['currentusernew']['userID']."'";					
			 $result = mysqli_query($conn,$query);					
...
<div class="table-responsive">    
    ...
    <tbody>
	 ...
	 //$romanValues=array(			'I' => 1,			'II' => 2,			'III' => 3,			'IX' => 4,			'V' => 5,			'VI' => 6,			'VII' => 7,			'VIII' => 8,			'IX' => 9,			'X' => 10,			'XI' => 11,		);		foreach ($romanValues as $key=>$chapterid){						if ($currentUser['grade']==$key) {				
		...
		$sqlsubject = "SELECT * FROM `knowledgekombat_chapter_time` kct INNER JOIN `chapter` c ON kct.`chapterID` = c.`chapterID` INNER JOIN `subject` s ON c.`subjectID` = s.`subjectID`  WHERE `userID` ='".$_SESSION['currentusernew']['userID']."' GROUP BY s.`subjectID`  ORDER BY s.`sequence` ASC";
	...
	<tr>
	 ...
	 <?php	 
		...
		$sql = "SELECT * FROM `knowledgekombat_chapter_time` kct INNER JOIN `chapter` c ON kct.`chapterID` = c.`chapterID` INNER JOIN `subject` s ON c.`subjectID` = s.`subjectID`  WHERE `userID` ='".$_SESSION['currentusernew']['userID']."' AND s.subject='".$subject."'   ORDER BY s.`sequence` ASC";
	...
	<td style="width: 400px"><?=$row1['chapter']; ?></td>
		...
		<?php 	
			 $progresssql = "select ROUND(SUM(progress)/COUNT(progress)) as  progress  from knowledgekombat_chapter_time  where chapterID ='".$row1['chapterID']."' and userID='".$_SESSION['currentusernew']['userID']."'";		
		...
		</td>		<?php $timesql = "select TIME_FORMAT(SEC_TO_TIME(`time`),'%HH : %iM : %SS') AS time from knowledgekombat_chapter_time  where chapterID ='".$row1['chapterID']."' and userID='".$_SESSION['currentusernew']['userID']."'";		$timeresult = mysqli_query($conn,$timesql);			$timerow = mysqli_fetch_array($timeresult);	   ?>	   	   		
        <td style="width: 105px;"><?=$timerow['time']; ?></td>
         <td class="text-center" style="width: 105px;">
		<span class="score"><?php 	
			$scoresql = "select ROUND(SUM(learningCurrency)/COUNT(learningCurrency)) as  learningCurrency  from knowledgekombat_chapter_time  where chapterID ='".$row1['chapterID']."' and userID='".$_SESSION['currentusernew']['userID']."'";		
        ...
        <td style="width: 105px;"><?php 	
            ...
            FROM knowledgekombat_chapter_time where chapterID ='".$row1['chapterID']."') ) AS rank FROM knowledgekombat_chapter_time where chapterID ='".$row1['chapterID']."' and userID='".$_SESSION['currentusernew']['userID']."'";
			//echo $ranksql;
